...
detect_plugstack_config() {
  ...
  configured_plugin="$(echo "$effective_content" | awk '{for (i=1; i<=NF; i++) if ($i ~ /mqss.*spank.*\.so$/) print $i}' | tail -n 1 || true)"
  if [[ -z "$configured_plugin" ]]; then
    echo "WARNING: No MQSS SPANK plugin entry found in $plugstack_path" >&2
    return 0
  fi
...
check_daemon_logs_for_hooks() {
  ...
  if grep -Fq "[mqss_spank_plugin] hook=" <<<"$daemon_log"; then
    echo "INFO: Hook logs found in slurmd journal (not forwarded to srun output)." >&2
    grep -F "[mqss_spank_plugin] hook=" <<<"$daemon_log" >&2 || true
    return 0
  fi
...
usage() {
  cat <<'EOF'
Usage: test/test_hook_logs.sh [options]

Runs a Slurm smoke test and checks for mqss_spank_plugin hook log lines.
...
Examples:
  ...
  test/test_hook_logs.sh --partition mqss
  test/test_hook_logs.sh --partition mqss --hook-mode full
  test/test_hook_logs.sh --partition mqss --require-all-hooks
EOF
}
...
for hook_name in "${expected_hooks[@]}"; do
  if grep -Fq "[mqss_spank_plugin] hook=${hook_name}" "$tmp_log_file"; then
