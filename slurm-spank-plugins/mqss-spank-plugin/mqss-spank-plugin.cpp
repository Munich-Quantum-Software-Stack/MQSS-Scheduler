...
// clang-format off
/** @file
 * @brief Slurm SPANK plugin for MQSS quantum-computer access control.
 *
 * This plugin provides HPC administrators with controlled injection of MQSS
 ...
 *   1. srun command-line options (--mqss-base-url, --mqss-tokens-file, etc.)
 ...
 * @code
 * required /usr/lib/slurm/mqss-spank-plugin.so partitions=quantum mqss_base_url=https://mqss.example.org mqss_tokens_file=/etc/mqss/tokens.json mqss_qc_alias=emerald
 * @endcode
 *
 * Supported plugstack arguments, srun options, and mapped env variables:
 *   - mqss_base_url     /  --mqss-base-url     → MQSS_BASE_URL
 *   - mqss_tokens_file  /  --mqss-tokens-file  → MQSS_TOKENS_FILE
 *   - mqss_qc_id        /  --mqss-qc-id        → MQSS_QC_ID
 *   - mqss_qc_alias     /  --mqss-qc-alias     → MQSS_QC_ALIAS
 *
 * Additional plugstack-only arguments:
 *   - partitions=quantum,quantum-dev (comma-separated, restricts activation)
 *   - mqss_license_prefix=mqss_qc_ (prefix used to derive the expected Slurm
 *     license name from MQSS_QC_ALIAS; default: "mqss_qc_")
 *   - mqss_require_license=1 (reject jobs whose Slurm license request does
 ...
 *
 * When `MQSS_QC_ALIAS` is resolved, the plugin can optionally check that the
 ...
 * `<mqss_license_prefix><alias>`. This is a *Slurm* license — a
 * capacity-limiting scheduler resource configured via `Licenses=` in
 * slurm.conf — used to bound concurrent access to a QC. This matters most
 * for on-premise QCs, which typically front single-tenant hardware, so
 * uncontrolled Slurm-side concurrency puts unnecessary pressure on the
 * device's own queue. Since aliases may contain characters reserved by
 ...
 * only logs a warning; `mqss_require_license=1` fails the task at launch
 ...
 *
 * Separately from this license-based capacity check, a site may also declare
 * `Gres=qpu:<alias>:<n>` per node (see `slurm-config/slurm.conf` /
 * `gres.conf` in this plugin's docker-testbed) so `sinfo`/`scontrol show
 * node` report QPU availability per node. That Gres declaration is purely
 * informational for now — Slurm already auto-enforces Gres capacity on its
 * own, and this plugin does not currently cross-check `--gres=qpu:...`
...
struct Config_mapping {
  /// Key expected in plugstack.conf (e.g. "mqss_base_url").
  std::string_view key;
  /// Environment variable to set in the job (e.g. "MQSS_BASE_URL").
  std::string_view env_var;
  /// srun option long name (e.g. "mqss-base-url").
...
constexpr std::array<Config_mapping, 4> K_CONFIG_MAPPINGS = {{
    {.key = "mqss_base_url",
     .env_var = "MQSS_BASE_URL",
     .option_name = "mqss-base-url",
     .option_usage = "MQSS scheduler base URL"},
    {.key = "mqss_tokens_file",
     .env_var = "MQSS_TOKENS_FILE",
     .option_name = "mqss-tokens-file",
     .option_usage = "Path to MQSS tokens file"},
    {.key = "mqss_qc_id",
     .env_var = "MQSS_QC_ID",
     .option_name = "mqss-qc-id",
     .option_usage = "Quantum computer ID"},
    {.key = "mqss_qc_alias",
     .env_var = "MQSS_QC_ALIAS",
     .option_name = "mqss-qc-alias",
     .option_usage = "Quantum computer alias"},
}};

/**
 * @brief Parses plugstack.conf arguments, processes srun options, and injects
 * MQSS environment variables into Slurm jobs.
 *
 * Lifecycle:
 ...
 *      `--mqss-*` srun command-line options.
 ...
 *      whatever consumes MQSS_* downstream (e.g. a QDMI device or the MQSS
 *      scheduler client).
 ...
 *      QC alias.
 */
class MQSSSpankConfigManager final {
public:
  /**
  ...
  void parse_plugstack_args(const int ac, char **av) {
    for (int i = 0; i < ac; ++i) {
      ...
      if (eq_pos == std::string_view::npos || eq_pos == 0 ||
          eq_pos + 1 >= arg.size()) {
        slurm_spank_log(
            "[mqss_spank_plugin] warning: ignoring malformed plugstack "
      ...
      if (!matched && key == "mqss_license_prefix") {
        license_prefix_ = std::string{value};
        matched = true;
      }

      if (!matched && key == "mqss_require_license") {
        require_license_ = parse_bool_arg(value, "mqss_require_license");
        matched = true;
      }

      if (!matched) {
        slurm_spank_log(
            "[mqss_spank_plugin] warning: unknown plugstack argument "
            "'%.*s'",
  ...
  /**
   * @brief Register srun command-line options for all MQSS configuration
   * keys.
  ...
  int register_options(spank_t spank) {
    for (std::size_t i = 0; i < K_CONFIG_MAPPINGS.size(); ++i) {
      ...
      srun_option_defs_[i] = {
          ...
          .cb = &MQSSSpankConfigManager::option_callback,
      };
      ...
      if (rc != ESPANK_SUCCESS) {
        slurm_spank_log(
            "[mqss_spank_plugin] error: failed to register option --%s: %s",
  ...
  [[nodiscard]] int inject_environment(spank_t spank) const {
    for (std::size_t i = 0; i < K_CONFIG_MAPPINGS.size(); ++i) {
      ...
      if (rc != ESPANK_SUCCESS && rc != ESPANK_ENV_EXISTS) {
        slurm_spank_log("[mqss_spank_plugin] error: failed to set %s: %s",
      ...
      if (rc == ESPANK_ENV_EXISTS) {
        slurm_debug(
            "[mqss_spank_plugin] %s already set by user, keeping user value",
            env_name.c_str());
      } else {
        const auto *source =
            srun_values_[i].has_value() ? "srun option" : "plugstack config";
        slurm_debug("[mqss_spank_plugin] set %s from %s", env_name.c_str(),
                    source);
      }
    }
    return ESPANK_SUCCESS;
   ...
   *   1. `MQSS_BASE_URL` must be set.
   *   2. `MQSS_TOKEN` and `MQSS_TOKENS_FILE` must not both be set.
   *
   * Deeper semantic/runtime validation is intentionally deferred to whatever
   * consumes these variables downstream.
   *
   * @param spank SPANK handle for the current hook invocation.
   * @return `ESPANK_SUCCESS` if valid, `ESPANK_ERROR` on conflict.
   */
  [[nodiscard]] static int validate_environment(spank_t spank) {
    const bool has_base_url = env_is_set(spank, "MQSS_BASE_URL");
    const bool has_token = env_is_set(spank, "MQSS_TOKEN");
    const bool has_tokens_file = env_is_set(spank, "MQSS_TOKENS_FILE");

    if (!has_base_url) {
      slurm_spank_log(
          "[mqss_spank_plugin] error: missing required MQSS_BASE_URL in job "
          "environment");
      return ESPANK_ERROR;
    }
    ...
    if (has_token && has_tokens_file) {
      slurm_spank_log(
          "[mqss_spank_plugin] error: conflicting auth — both "
          "MQSS_TOKEN and MQSS_TOKENS_FILE are set in job environment");
      return ESPANK_ERROR;
    }
    ...
    slurm_debug("[mqss_spank_plugin] validation status=ok base_url=%s "
                "auth_source=%s",
   ...
   * capacity-limiting scheduler resource.
   *
   * The expected license name is `<mqss_license_prefix><alias>`, with `:`
   ...
   *   - No `MQSS_QC_ALIAS` is resolved (`MQSS_QC_ID`-only jobs are not
   *     checked; raw IDs are not reliably convertible into license names).
   *   - `SLURM_JOB_LICENSES` is absent and `mqss_require_license` is
   *     disabled. Absence means either Slurm < 23.02 (where the variable
   *     does not exist) or that the user requested no `--licenses`; these
   *     cases cannot be distinguished. With `mqss_require_license` enabled,
   *     either case is rejected. A value too long to fit the lookup buffer
   *     is treated as a mismatch instead (fail closed, not skipped).
   *
   ...
   *   `mqss_require_license` is enabled and the expected license is missing.
   */
  [[nodiscard]] int validate_license_alignment(spank_t spank) const {
    const auto alias = get_spank_env(spank, "MQSS_QC_ALIAS");
    if (!alias.has_value() || alias->empty()) {
      slurm_debug(
          "[mqss_spank_plugin] no MQSS_QC_ALIAS resolved, skipping Slurm "
          "license alignment check (MQSS_QC_ID-only jobs are not checked)");
      return ESPANK_SUCCESS;
    }
    ...
    // would fail open under mqss_require_license.
    ...
    if (licenses_rc != ESPANK_SUCCESS && licenses_rc != ESPANK_NOSPACE) {
      if (!require_license_) {
        slurm_debug(
            "[mqss_spank_plugin] SLURM_JOB_LICENSES not present in job "
      ...
      slurm_spank_log(
          "[mqss_spank_plugin] error: missing required Slurm license '%s' "
          "for QC alias '%s' — resubmit with --licenses=%s:<n> "
          "(mqss_require_license is enabled)",
    ...
    if (licenses_rc == ESPANK_NOSPACE) {
      ...
      // mqss_require_license).
      slurm_debug(
          "[mqss_spank_plugin] SLURM_JOB_LICENSES exceeds %d bytes, cannot "
    ...
    if (aligned) {
      slurm_debug(
          "[mqss_spank_plugin] Slurm license alignment ok: '%s' requested "
          "for QC alias '%s'",
    ...
    if (!require_license_) {
      slurm_spank_log(
          "[mqss_spank_plugin] warning: Slurm license not requested for QC "
          ...
          "mqss_require_license=1 to enforce this as a hard requirement",
          alias->c_str(), expected_license.c_str());
      return ESPANK_SUCCESS;
    }

    slurm_spank_log(
        "[mqss_spank_plugin] error: missing required Slurm license '%s' for "
  ...
  [[nodiscard]] bool is_active_for_job(spank_t spank) const {
    ...
    if (!partition.has_value()) {
      slurm_debug("[mqss_spank_plugin] could not determine job partition, "
                  "proceeding anyway");
      return true;
    }
    ...
    slurm_debug(
        "[mqss_spank_plugin] partition '%s' not in active list, skipping",
        partition.value().c_str());
    return false;
  }

  /**
   * @brief Check accessibility of the MQSS_TOKENS_FILE if configured.
   *
  ...
  static void check_tokens_file_access(spank_t spank) {
    const auto path = get_spank_env(spank, "MQSS_TOKENS_FILE");
    if (!path.has_value()) {
      return;
    }
    ...
    if (!std::filesystem::is_regular_file(tokens_path, ec) || ec) {
      slurm_spank_log("[mqss_spank_plugin] warning: MQSS_TOKENS_FILE '%s' is "
   ...
   *
   * Format:
   *   [mqss_spank_plugin] job=<id> partition=<name> base_url=<set|unset>
  ...
  void emit_diagnostics(spank_t spank) const {
    ...
    if (has_tokens_file) {
      if (const auto path = get_spank_env(spank, "MQSS_TOKENS_FILE")) {
        std::error_code ec;
    ...
    if (const auto alias = get_spank_env(spank, "MQSS_QC_ALIAS");
    ...
    const auto msg = std::format(
        "[mqss_spank_plugin] job={} partition={} base_url={} auth={} "
  ...
  void log_parsed_config() const {
    ...
    slurm_debug("[mqss_spank_plugin] parsed %d plugstack argument(s), "
...
private:
  /**
   * @brief Callback invoked by Slurm when a user provides an --mqss-* option.
   *
  ...
  [[nodiscard]] static bool parse_bool_arg(std::string_view value,
    ...
    slurm_spank_log(
        "[mqss_spank_plugin] warning: unrecognized value '%.*s' for %s, "
  ...
  /// MQSS_QC_ALIAS. Configurable via the `mqss_license_prefix` plugstack
  /// argument (default: "mqss_qc_").
  std::optional<std::string> license_prefix_;

  /// When true, jobs that resolve an MQSS_QC_ALIAS but do not request the
  /// corresponding Slurm license via --licenses are rejected outright
  /// instead of only receiving a warning. Configurable via
  /// `mqss_require_license` (default: false/off).
...
MQSSSpankConfigManager g_config{}; // NOLINT(*-avoid-non-const-global-variables)

// Static callback implementation — stores the srun option value.
int MQSSSpankConfigManager::option_callback(const int val, const char *optarg,
                                            int /* remote */) {
  if (val < 0 || static_cast<std::size_t>(val) >= K_CONFIG_MAPPINGS.size()) {
    slurm_spank_log("[mqss_spank_plugin] error: invalid option id %d", val);
    return -1;
  }
  if (optarg == nullptr || optarg[0] == '\0') {
    slurm_spank_log(
        "[mqss_spank_plugin] error: empty value for --%s",
  ...
  slurm_debug("[mqss_spank_plugin] srun option --%s provided",
              K_CONFIG_MAPPINGS[static_cast<std::size_t>(val)].option_name);
  return 0;
}
...
void Emit_hook_log(const char *hook_name) {
  slurm_spank_log("[mqss_spank_plugin] hook=%s", hook_name);
}
...
extern const char plugin_name[] = "mqss_spank_plugin";
 ...
 *
 * Injects MQSS environment variables into the job environment and performs
...
int slurm_spank_task_init(spank_t spank, const int /* ac */, char ** /* av */) {
  ...
  const auto validation_rc = MQSSSpankConfigManager::validate_environment(spank);
  const auto license_validation_rc = g_config.validate_license_alignment(spank);

  // Auth file accessibility check (warning only).
  MQSSSpankConfigManager::check_tokens_file_access(spank);
